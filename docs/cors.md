...
CORS 全称 “**跨域资源共享（Cross-origin resource）**”，它突破了一个请求在浏览器发出只能在同源的情况下向服务器获取数据的限制。

## 浏览器还是服务器的限制？

先思考下，CORS 是浏览器端还是服务器端的限制？为了更好的说明这个问题，从一个示例看一看。

**index.html**

创建 index.html 使用 fetch 调用 http://127.0.0.1:3011
